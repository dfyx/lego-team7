 ...
 */
public class Engine {
    private static final float DISTANCE_SCALE_FACTOR =
            (float) (38.0 * Math.PI / 360);
    private static final float ANGLE_SCALE_FACTOR = (float) (0.105);

    private static final MotorPortProxy LEFT_MOTOR_PORT = new MotorPortProxy(
            MotorPort.A);
    private static final MotorPortProxy RIGHT_MOTOR_PORT = new MotorPortProxy(
            MotorPort.B);
    private static final NXTRegulatedMotor LEFT_MOTOR = new NXTRegulatedMotor(
            LEFT_MOTOR_PORT);
    private static final NXTRegulatedMotor RIGHT_MOTOR = new NXTRegulatedMotor(
            RIGHT_MOTOR_PORT);

    protected int oldLeftSpeed = 0;
    protected int oldRightSpeed = 0;
    ...
    private Engine() {
        ...
        // Measure speed
        calibrationMaxSpeed =
                (int) (0.9 * Math.min(LEFT_MOTOR.getRotationSpeed(),
                        RIGHT_MOTOR.getRotationSpeed()));
     ...
     * example call {@link #stop() stop} before the next call of
     * {@link #commit() commit}.
     * 
    ...
    public void move(int speed, int direction) {
        System.out.println("MOVE WITH " + speed);
        ...
        if (speed < -MAX || speed > MAX) {
            throw new IllegalArgumentException("Speed must be between " + -MAX
                    + " and " + MAX);
        }
    ...
    public float estimateDistance() {
        return((LEFT_MOTOR.getTachoCount() - lastTachoLeft
                + RIGHT_MOTOR.getTachoCount() - lastTachoRight) / 2 * DISTANCE_SCALE_FACTOR);
    }
